...
//NRF24L01的SPI接速率为0~8Mbps
#define SoftWareSPI		1
#define HardWareSPI		hspi1

// 与无线转USB串口的默认参数配对使用时，需要在 NRF24L01.h 中将以下三个参数按如下配置。
// RF射频通道为 100，对应无线转USB串口频率为2.5GHz。
// RF地址宽度 5byte
// RF数据宽度 31byte
#define NRF_RF_CH       100					// RF射频通道 0~125
#define	NRF_ADDR_WIDTH  5                   // RF地址宽度 1~5byte
#define NRF_DATA_WIDTH  4                  // RF数据宽度 1~32byte


//-------宏定义引脚--------
#define NRF_CE_PIN          GPIOB,GPIO_PIN_1
#define NRF_CSN_PIN         GPIOB,GPIO_PIN_10

#if SoftWareSPI
#define NRF_SCK_PIN         GPIOA,GPIO_PIN_5
#define NRF_MOSI_PIN        GPIOA,GPIO_PIN_7
#define NRF_MISO_PIN        GPIOA,GPIO_PIN_6
#endif

#define NRF_IRQ_PIN         GPIOB,GPIO_PIN_0

#define NRF_CE(x)   HAL_GPIO_WritePin(NRF_CE_PIN,x)
#define NRF_CSN(x)  HAL_GPIO_WritePin(NRF_CSN_PIN,x)
#define NRF_MOSI(x) HAL_GPIO_WritePin(NRF_MOSI_PIN,x)
#define NRF_SCK(x)  HAL_GPIO_WritePin(NRF_SCK_PIN,x)

#define NRF_MISO    HAL_GPIO_ReadPin(NRF_MISO_PIN)
#define NRF_IRQ     HAL_GPIO_ReadPin(NRF_IRQ_PIN)
